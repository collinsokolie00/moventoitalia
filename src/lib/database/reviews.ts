...
const listCachedReviews=unstable_cache(async (locale:Locale="en") => { const snapshot=await adminDb.collection("reviews").get(); return snapshot.docs.map(doc=>mapReview(doc,locale)).sort((a,b)=>a.displayOrder-b.displayOrder||a.customerName.localeCompare(b.customerName)); },["reviews"],{revalidate:300,tags:["reviews"]});
export async function listReviews(locale:Locale="en") { return listCachedReviews(locale); }
