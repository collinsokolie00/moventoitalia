...
export async function listLocalizedServices():Promise<LocalizedService[]> { const snapshot=await adminDb.collection(SERVICES_COLLECTION).get();return snapshot.docs.map(document=>{const data=document.data();return{id:document.id,slug:data.slug??"",title:normalizeLocalized(data.title,""),subtitle:normalizeLocalized(data.subtitle,""),description:normalizeLocalized(data.description,""),features:normalizeLocalized(data.features,[]),icon:data.icon??"Truck",image:data.image??"",seoTitle:normalizeLocalized(data.seoTitle,""),seoDescription:normalizeLocalized(data.seoDescription,""),published:Boolean(data.published),displayOrder:Number(data.displayOrder??0)};}).sort((a,b)=>a.displayOrder-b.displayOrder||a.title.en.localeCompare(b.title.en));}
