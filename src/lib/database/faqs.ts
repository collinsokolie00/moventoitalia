...
const listCachedFAQs=unstable_cache(async (locale:Locale="en")=>{const snapshot=await adminDb.collection(FAQS_COLLECTION).get();return snapshot.docs.map(document=>mapFAQ(document,locale)).sort((a,b)=>a.displayOrder-b.displayOrder||a.question.localeCompare(b.question));},["faqs"],{revalidate:300,tags:["faqs"]});
export async function listFAQs(locale:Locale="en"){return listCachedFAQs(locale);}
